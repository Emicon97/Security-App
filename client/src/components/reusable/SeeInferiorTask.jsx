          ...
          <div className="flex">
            ...
            <div className="h-[40px] my-auto flex flex-col">
              <h3 className="flex font-semibold text-white">
                <p>{userDetails.name.charAt(0).toUpperCase()}</p>{userDetails.name.slice(1)} {' '}
                {userDetails.lastName.charAt(0).toUpperCase() + userDetails.lastName.slice(1)}
              </h3>
              <small className="text-white">{userDetails.email} | {userDetails.telephone}</small>
            </div>
          </div>
            ...
            (
              userTasks.map((task) => (
                <div key={task._id} 
                  ...
                  flex flex-col w-[250px] h-auto rounded-2xl p-2.5`}
                >
                  <div key={task._id}>
                    ...
                    <div className="flex justify-between">
                      <div className="flex items-center">
                        ...
                        <h4 className="italic ml-1 text-xs">Priority:{" "}{task.priority.charAt(0).toUpperCase() + task.priority.slice(1)}</h4>
                      </div>
                      <span className="flex items-center gap-1.5 italic">
                        {task.status.charAt(0).toUpperCase() + task.status.slice(1)}
                        <p className={`${task.status === "done" ? "h-4 w-4 bg-green-500" : task.status === "left" ? "h-4 w-4 bg-red-500" : "h-4 w-4 bg-yellow-500"} rounded-full`}></p>
                      </span>
                    </div>
                  </div>
                  <div className="flex flex-row items-center">
                    <button title="See reports" onClick={(e) => {
